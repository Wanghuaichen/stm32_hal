...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  uint8_t i;
  uint8_t pl[][10]={"0-1000HZ","0-5000HZ","0-10000HZ"};
  uint8_t ch[20];
  // USER END
  ...
  case WM_KEY:
    ...
    case GUI_KEY_ENTER:
        if((((WM_KEY_INFO*)(pMsg->Data.p))->PressedCnt))
        {
            ...
            {
                shuchu.OnOff[1]=LISTBOX_GetSel(hWin_v[1]);
                WM_SetFocus(hWin_v[0]);
            }
            else if(hItem==hWin_v[2])
            {
                shuchu.Mode[1]=LISTBOX_GetSel(hWin_v[2]);
                WM_SetFocus(hWin_v[0]);
            }
        }
        WM_DefaultProc(pMsg);
    ...
    case GUI_KEY_LEFT:
        ...
        {
            hItem=WM_GetFocussedWindow();
            if(hItem==WM_GetDialogItem(pMsg->hWin, ID_LISTBOX_0));
            {
